...
func (ds *DataStore) getMetadatas(ctx context.Context, metadataQuery *sql.Stmt, args []any) ([]*content.Metadata, error) {
	...
	for result.Next() {
		...
		traitResults, err := traitsQuery.QueryContext(ctx, metadata.Id)
		if err != nil {
			return nil, err
		}
		traits := make([]string, 0)
		for traitResults.Next() {
			var trait string
			err = traitResults.Scan(&trait)
			if err != nil {
				traitResults.Close()
				return nil, err
			}
		...
		traitResults.Close()
		categoryResults, err := categoriesQuery.QueryContext(ctx, metadata.Id)
		if err != nil {
			return nil, err
		}
		...
		for categoryResults.Next() {
			var category string
			err = categoryResults.Scan(&category)
			if err != nil {
