...
func main() {
	...
	workflowQueueConnection, err := clients.NewClientConnection(cfg.ClientEndPoints.WorkflowQueueApiAddress)
	if err != nil {
		slog.Error("failed to get content connection", slog.Any("error", err))
		os.Exit(1)
	}
	workflowQueueClient := protoworkflow.NewWorkflowQueueServiceClient(workflowQueueConnection)

	svc := workflow.NewAuthorizationService(permissions, ds, workflow.NewService(cfg, ds, cfg.Security.ServiceAccountId, cfg.Security.ServiceAccountToken, objectStore, permissions, contentClient, workflowQueueClient))
