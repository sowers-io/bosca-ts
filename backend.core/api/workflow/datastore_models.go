...
func (ds *DataStore) GetWorkflowActivityModels(ctx context.Context, activityId int64) ([]*model.WorkflowActivityModel, error) {
	rows, err := ds.db.QueryContext(ctx, "select model_id, configuration from workflow_activity_models where activity_id = $1", activityId)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
