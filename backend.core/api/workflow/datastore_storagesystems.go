...
import (
	model "bosca.io/api/protobuf/bosca/workflow"
	"context"
	"database/sql"
	"encoding/json"
	"strings"
...
func (ds *DataStore) GetWorkflowActivityStorageSystems(ctx context.Context, activityId int64) ([]*model.WorkflowActivityStorageSystem, error) {
	results, err := ds.db.QueryContext(ctx, "select storage_system_id, configuration from workflow_activity_storage_systems where activity_id = $1", activityId)
	if err != nil {
		return nil, err
	}
	defer results.Close()
