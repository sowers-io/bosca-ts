...
export abstract class BookActivity extends Activity {
  ...
  abstract newBookExecutor(): BookExecutor

  newJobExecutor(job: Job, definition: WorkflowJob): ActivityJobExecutor<any> {
    return new Executor(this.newBookExecutor(), this, job, definition)
  }
}
...
class Executor extends ActivityJobExecutor<BookActivity> {
  ...
  async execute() {
    ...
    try {
      ...
      await this.executor.execute(source, systemId, metadata, this.definition, book)
    } catch (e) {
      throw e
    } finally {
